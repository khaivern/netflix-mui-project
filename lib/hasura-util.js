...
export const getVideoDataByUserId =  async ({videoId, userId, token}) => {
  ...
  try {
    const response = await fetchGraphQL(operationsDoc, "isNewVideoRecord", {videoId, userId}, token)
    if(response.errors) {
      throw new Error("Received error from Hasura: ", response.errors[0].message)
    }
...
export const updateVideoData = async ({favourited, userId, videoId, token}) => {
  ...
  `;
  try {
    const response = await fetchGraphQL(operationsDoc, "updateVideo", {favourited, userId, videoId}, token)
    if(response.errors) {
      throw new Error("Hasura error, failed when updating video record", response.errors[0].message);
    }
...
export const insertVideoData = async ({favourited, userId, videoId, token}) => {
  ...
  `;
  try {
    const response = await fetchGraphQL(operationsDoc, "insertVideo", {favourited, userId, videoId}, token);
